    ...
    <div className="mx-auto max-w-4xl p-6">
      ...
      <div className="space-y-8">
        {posts.map((post) => (
          <article
            key={post.slug}
            className="border-b border-[var(--grid)] pb-8 last:border-0 group/article hover:border-[var(--accent)] transition-colors duration-150"
          >
            <Link
              href={`/notes/${post.slug}`}
              className="group block cursor-pointer transition-all"
            >
              <h2 className="mb-2 font-mono text-2xl uppercase group-hover:text-[var(--accent)] transition-colors duration-150">
                {post.title}
              </h2>
